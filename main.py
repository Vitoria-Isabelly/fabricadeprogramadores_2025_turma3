...
def criar_novo_usuario_e_nota():
    ...
    novo_usuario = Usuario( 
        ...
        email="Vitoriaiasaabelly211a07@email.com",
...
def ler_dados (nome):
    ...
    if user:
        ...
        for nota in user.notas :
            print(f" - Título: {nota.conteudo} (ID: {nota.id})")
    else:
...
def atualizar_nota(id_nota):
    ...
    if nota_para_editar:
        print(f"Título original: '{nota_para_editar.conteudo}'")

        nota_para_editar.conteudo = "Lista de anotações ATULIZADA!"

        db.commit()
        print(f"Título novo: '{nota_para_editar.conteudo}'")
